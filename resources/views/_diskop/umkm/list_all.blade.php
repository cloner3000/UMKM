...
@section('title_page')
    <h2>Umkm Baru
        <small>Daftar Umkm yang telah bergabung</small>
    </h2>
@endsection
@section('content')
                    ...
                    <div class="table-responsive">
                        <table class="table table-bordered table-striped table-hover js-basic-example dataTable"
                            ...
                            @foreach($umkm as $data)
                                <tr>
                                    <td><img src="{{asset($data->avatar)}}" class="img-circle" alt=""></td>
                                    <td>{{$data->nama}}</td>
                                    <td>{{$data->nama_pemilik}}</td>
                                    <td>{{\Carbon\Carbon::parse($data->tgl_berdiri)->format('d-m-Y')}}</td>
                                    <td>
